...
def file_type_check(filename: str, expected_type: t.Union[str, list]):
  ...
  Args:
      ...
      expected_type (t.Union[str, list]): either a string with the expected filetype or a list of strings with expected file types 
  """
  # get the extension from filename
  extension = filename[::-1].strip().split('.')[0].lower()[::-1]
  try:
